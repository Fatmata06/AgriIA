...
async function getWeather(city){
  try {
    ...
    document.getElementById('weather-alerts').textContent = `Température: ${data.temp}°C, Conditions: ${data.description}`;
  } catch(err){
    console.error("Erreur météo:", err);
    document.getElementById('weather-alerts').textContent = "Impossible de récupérer la météo.";
  }
}

// ==================== Upload et diagnostic plante ====================
    ...
    try {
      ...
      if(data.plant_name){
        document.getElementById('diagnosis').textContent = 
          `Plante probable: ${data.plant_name}\nMaladie possible: ${data.disease}\nConfiance: ${data.probability}%\nDescription: ${data.description}\nURL: ${data.url || '—'}`;
      } else {
